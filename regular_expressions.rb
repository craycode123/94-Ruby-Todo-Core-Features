...
def grab_ssn(string)
  #.join to make it string from array
  string.match(/(\d{3}-\d{2}-\d{4})/).captures.join if has_ssn?(string)
end
...
def grab_all_ssns(string)
  string.scan(/(\d{3}.?\d{2}.?\d{4})/).join(",").split(",")
end
...
def format_ssns(string)
  if grab_all_ssns(string) != []
    arr = []
    grab_all_ssns(string).each do |ssn|
      arr << ssn.gsub(/(\d{3}).?(\d{2}).?(\d{4})/, '\1-\2-\3')
    end
    arr.join(", ")
  else
    string
